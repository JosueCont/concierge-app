...
const vacationScreen = (props) => {
  ...
  useEffect(() => {
    if (props.user && props.user.userProfile) {
      getRecordVacations(props.user.userProfile.id);
  ...
  const getRecordVacations = async () => {
    setRecordVacations(0);
    try {
      setModalLoading(true);
      let response = await ApiApp.getRecordVacations(props.user.userProfile.id);
      if (response.data) setRecordVacations(response.data?.vacations_pending);
    } catch (error) {
      ...
      <>
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              props.navigation.navigate("VacationRequestScreen", {
                vacations_pending: recordVacations,
              })
            }
          >
